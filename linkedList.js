...
// Тестируем ошибки
try {
  list.insert(10, 100); // Недопустимая позиция
} catch (e) {
  console.log("Ошибка при вставке:", e.message); // Ожидаемый результат: Недопустимое место для вставки элемента
}

try {
  list.removeAt(10); // Недопустимая позиция
} catch (e) {
  console.log("Ошибка при удалении:", e.message); // Ожидаемый результат: Недопустимое место для удаления элемента
}

try {
  list.update(10, 100); // Недопустимая позиция
} catch (e) {
  console.log("Ошибка при обновлении:", e.message); // Ожидаемый результат: Недопустимы индекс
}

try {
  list.removeFirst(); // Удаление из пустого списка
} catch (e) {
  console.log("Ошибка при удалении первого элемента:", e.message); // Ожидаемый результат: Список пуст
}

try {
  list.removeLast(); // Удаление из пустого списка
} catch (e) {
  console.log("Ошибка при удалении последнего элемента:", e.message); // Ожидаемый результат: Список пуст
}
